...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if !errors.is_empty() {
        println!("\n🔍 Detailed errors:");
        for error in &errors[..errors.len().min(10)] { // Show max 10 errors
            println!("   {}", error);
        }
        if errors.len() > 10 {
...
fn verify_gem(gem_path: &Path) -> Result<GemInfo, Box<dyn std::error::Error>> {
    let mut package = Package::open(gem_path)
        .map_err(|e| format!("Failed to open gem: {}", e))?;
...
fn find_gem_files(dir: &Path) -> Result<Vec<PathBuf>, Box<dyn std::error::Error>> {
...
fn get_default_gem_cache_paths() -> Result<Vec<PathBuf>, Box<dyn std::error::Error>> {
    ...
    for entry in fs::read_dir(&gem_dir)? {
        let entry = entry?;
        let path = entry.path();
    ...
    if cache_paths.is_empty() {
        ...
        eprintln!("Example: {} /path/to/gems /another/path", env::args().next().unwrap_or_default());
    }
