...
/// Assuming the binary is called something like `rvw@1.2.3(.exe)`, compute the `@1.2.3(.exe)` part
/// so that we can preferentially find `rv@1.2.3(.exe)`, for folks who like managing multiple
/// installs in this way.
fn get_rvw_suffix(current_exe: &Path) -> Option<&str> {
    let os_file_name = current_exe.file_name()?;
    let file_name_str = os_file_name.to_str()?;
    file_name_str.strip_prefix("rvw")
}

/// Gets the path to `rv`, given info about `rvw`
fn get_rv_path(current_exe_parent: &Path, rvw_suffix: Option<&str>) -> std::io::Result<PathBuf> {
    // First try to find a matching suffixed `rv`, e.g. `rv@1.2.3(.exe)`
    let rv_with_suffix = rvw_suffix.map(|suffix| current_exe_parent.join(format!("rv{suffix}")));
...
fn run() -> std::io::Result<ExitStatus> {
    ...
    let Some(bin) = current_exe.parent() else {
        return Err(std::io::Error::new(
            ...
            "Could not determine the location of the `rvw` binary",
        ));
    };
    let rvw_suffix = get_rvw_suffix(&current_exe);
    let rv = get_rv_path(bin, rvw_suffix)?;
